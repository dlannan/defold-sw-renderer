...
static std::vector<scene_blinn_t> read_blinn_materials(FILE *file) {
...
static std::vector<scene_pbrm_t> read_pbrm_materials(FILE *file) {
    std::vector<scene_pbrm_t> materials;
    int num_materials;
    int items;
    int i;
...
static std::vector<scene_pbrs_t> read_pbrs_materials(FILE *file) {
    std::vector<scene_pbrs_t> materials;
    int num_materials;
    int items;
    int i;
...
static scene_t *create_blinn_scene(scene_light_t *scene_light,
                                   std::vector<scene_blinn_t> scene_materials,
...
static scene_t *create_pbrm_scene(scene_light_t *scene_light,
                                  std::vector<scene_pbrm_t> scene_materials,
                                  std::vector<scene_transform_t> scene_transforms,
                                  std::vector<scene_model_t> scene_models,
                                  mat4_t root_transform) {
...
static scene_t *create_pbrs_scene(scene_light_t *scene_light,
                                  std::vector<scene_pbrs_t> scene_materials,
                                  std::vector<scene_transform_t> scene_transforms,
                                  std::vector<scene_model_t> scene_models,
                                  mat4_t root_transform) {
    const char *env_name = wrap_path(scene_light->environment);
    ...
    for (i = 0; i < num_models; i++) {
        ...
        printf("pbrs scene_mode.mesh: %s\n", scene_model.mesh);
        printf("pbrs scene_mode.skeleton: %s\n", scene_model.skeleton);
